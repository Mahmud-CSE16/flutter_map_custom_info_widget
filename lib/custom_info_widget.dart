        ...
        child: ClipPath(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: Colors.white,
              boxShadow: [BoxShadow(blurRadius: 5.0,spreadRadius: 5.0,offset: Offset(0,5))]
            ),
...
class _InfoWidgetClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    Path path = Path();
    path.addRRect(RRect.fromLTRBR(0, 0, size.width, size.height - 15.0, Radius.circular(5.0)));
    path.moveTo(size.width / 2 - 8.0, size.height-15.0);
    //path.lineTo(size.width / 2 - 10, size.height - 10);
    path.lineTo(size.width / 2, size.height);
    path.lineTo(size.width / 2 + 8.0, size.height - 15.0);
    path.close();
    return path;
  }
